...
pub struct ConfigurationsAvailable {
    overrides: String,
    version: String,
    url: String,
...
base_topic!(ConfigurationsAvailable);

impl ConfigurationsAvailable {
    pub fn get_overrides(&self) -> String {
        self.overrides.to_owned()
    }
...
mod tests {
    ...
    fn test_deserialize() {
        ...
        topic_record.put("overrides", Value::String("overrides".to_string()));
        topic_record.put("version", Value::String("version".to_string()));
        topic_record.put("url", Value::String("url".to_string()));
        topic_record.put("schemaVersion", Value::String("schemaVersion".to_string()));
        ...
        for record in reader {
            let topic = from_value::<ConfigurationsAvailable>(&record.unwrap()).unwrap();

            assert_eq!(topic.get_overrides(), "overrides");
            assert_eq!(topic.get_version(), "version");
            assert_eq!(topic.get_url(), "url");
            assert_eq!(topic.get_schema_version(), "schemaVersion");
